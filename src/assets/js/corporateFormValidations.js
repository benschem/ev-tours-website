...
document.addEventListener("DOMContentLoaded", () => {
  const inputs = document.querySelectorAll("#fullName, #company, #email, #phone, #serviceType, #itinerary");
...
const setValidityMessage = (input) => {
  switch (input.id) {
    case "fullName":
      ...
      break;
    case "company":
      setCustomMessage(input, "company");
      break;
    case "email":
  ...
  switch (type) {
    ...
    case "company":
      if (userInput.tooShort) {
        input.setCustomValidity("The company name looks too short");
      } else {
        input.setCustomValidity("");
      }
      break;
